...
### for producing the diagonal units:
def diag(a, b):
    return [[a[i]+b[i] for i in range(len(a))], [a[-j-1]+b[j] for j in range(len(a))]]

diag_units = diag(rows, cols)  ## getting the diagonal units
...
def naked_twins(values):
    ...
    for unit in unitlist:
        ...
        for box in dplaces:
            for p in dplaces:
                if values[p] == values[box] and p!=box:
                    digits = values[box]
                    for peer in unit:
                        if peer!=p and peer!=box:
...
def grid_values(grid):
    ...
    # I like my method here more than the official solution.
...
def eliminate(values):
    ...
    for key in values:
        if len(values[key]) == 1:
            tempchar = values[key]
            for k in peers[key]:
                values[k] = values[k].replace(tempchar, "")
    return values
